...
cdd_w2 = NCH.look_up('CDD_CSS', gm_id=gm_id, vds=0.2, vsb=0.2, L=L23)
fp2 = 1/2/np.pi*gm_css3*(1+gmb_gm3)/(1+2*cdd_css3*2*(cdd_w2/cdd_w3))
print(f'Non dominant pole fp2 = {Float(fp2):!.1h}Hz') #%.2E Hz' % fp2)
...
s.vod = 50 # total integrated noise
s.T = 300 # Kelvin
s.fan_out = 0.5
...
def folded_cascode(s, d):
    ...
    for beta in d.beta[:-1]: # Exclude last point for removing issues with interpolation to close to upper boundary
        ...
        gm_id1_physical = interp1(beta_actual, d.gm_id1, kind='nearest')(beta)
        ...
        # Check hypothesis
        gamma = NCH.look_up('STH_GM', gm_id=gm_id1_physical, L=d.L1)/4/sp.constants.Boltzmann/s.T  #  vgs=vgs1
        m1.gamma.append(gamma)

    plt.figure()
    plt.plot(m1.gm_id1, m1.id1, label='L = %.1f um'%d.L1)
    plt.ylabel(r"$I_D$ [mA]")
